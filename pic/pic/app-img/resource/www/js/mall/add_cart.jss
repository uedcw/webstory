...
$(function(){
    ...
    $(document).on('click','#add_cart_dialog_but',function(){
    	...
    	if(!$(this).hasClass('disable')){
	    	cart.shopping(dialogItemDetail.itemIn.id, dialogItemDetail.itemIn.img, 'cart_item_'+dialogItemDetail.itemIn.id);
	        if(!$(this).hasClass('disable')){
	        	dialog.closeBox();
	            ...
	            }, dialogItemDetail.priceIn[dialogItemDetail.defaultPurity]);
	        }
    	}
    });
    ...
    if(buyQuantity.isInteger() && buyQuantity > 0){
    	...
    	if(allQuantity < parseFloat(dialogItemDetail.minQuantity)){
        	...
        	if(dialogItemDetail.is_pack == 1){
        		...
        		var curAddPriceIn = getNewPrice(dialogItemDetail.priceIn[dialogItemDetail.defaultPurity],  buyQuantity);
            	addOnClass('dialog_price_form_'+curAddPriceIn['id'], 'on');
            	...
            	var curPriceIn = getNewPrice(dialogItemDetail.priceIn[dialogItemDetail.defaultPurity], allQuantity);
