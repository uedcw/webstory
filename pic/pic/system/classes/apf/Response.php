...
class APF_Response {
    ...
    public function set_cookie($name, $value, $expire=0, $path=NULL, $domain=NULL, $secure=FALSE, $httponly=FALSE) {
        if (!$path) {
            $path = @APF::get_instance()->get_config(self::CONFIG_N_COOKIE_PATH);
        }
        if (!$domain) {
            $domain = @APF::get_instance()->get_config(self::CONFIG_N_COOKIE_DOMAIN);
        }
    ...
    public function set_content_type($content_type, $charset=NULL) {
        ...
        if (!$charset && preg_match('/^text/i', $content_type)) {
            $charset = @APF::get_instance()->get_config('charset');
            if (!$charset) {
                $charset = 'utf-8';
            }
        }
