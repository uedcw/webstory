...
apf_require_class('APF_AMQP_Factory');

abstract class APF_AMQP_Object {
    ...
    public function save() {
        ...
        try {
            ...
            $con = APF_AMQP_Factory::get_instance()->get_connection();
            $ex = new AMQPExchange($con, self::get_exchange());
            $ret = $ex->publish($msg, self::get_routing());
        } catch (Exception $e) {
            APF::get_instance()->get_debugger()->debug('AMPQ: Can not be published - ' .$e->getMessage());
        }
