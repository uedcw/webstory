...
abstract class MPF_Component {
    ...
    public function script_block_end($order=0) {
        ...
        MPF::get_instance()->register_script_block($content,$order);
        ob_end_clean();
    }
    /**
     * 获取页面框架名称
    ...
    protected function render($file) {
        ...
        // 预设请求类和响应类变量，其实大可不必
        $request = MPF::get_instance()->get_request();
        $response = MPF::get_instance()->get_response();
        //esi
        //if($this->params['mode']=='esi'&&strpos($_SERVER['Surrogate-Capability'],'ESI/1.0')!==false){echo "<esi:include src='/componetrender/?component=".str_replace('Component','',get_class($this))."'/>";return;}  //microhuang debug
        //bigpipe
        //if($this->params['mode']=='pipe'){echo "<div id='".$this->html_id."'></div>";return;}
        //default
        // 渲染？？？就是include，够高端吧。
        include($file);
    }
    ...
    public function component($class, $params=array()) {
        //if($params['mode']=='esi'&&stristr($_SERVER['Surrogate-Capability'],'esi')==false){echo "<esi:include src='/".str_replace('_','/',strtolower($class))."'/>";return;}//esi debug
        //if($params['mode']=='esi'&&strpos($_SERVER['Surrogate-Capability'],'ESI/1.0')!==false){echo "<esi:include src='/componetrender/?".str_replace('_','/',strtolower($class))."/ ttl="1h" maxwait="1000"'/>";return;}
        return MPF::get_instance()->component($this, $class, $params);
    }
