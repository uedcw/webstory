...
class MPF_Cache_Factory{
    ...
    {
        $type = MPF::get_instance()->get_config("type","cache");
        if($type=='memcache')
        {
    ...
    public function load_memcache($name='servers'){
        mpf_require_class($this->memcache_class);
        $memcache=new $this->memcache_class();
        $servers=MPF::get_instance()->get_config($name,"cache");
    ...
    public function fail($host,$port){
        ...
        if(110==$code||10060==$code){
            MPF::get_instance()->get_logger()->error('memcache '.$host.':'.$port.' failed '.$code);
            MPF::get_instance()->pf_benchmark_inc_begin('MPF_Cache_Factory::fail');
            MPF::get_instance()->pf_benchmark_inc_end('MPF_Cache_Factory::fail');
        }else{
            MPF::get_instance()->get_logger()->error('memcache '.$host.':'.$port.' failed '.$code.' '.$m);
        }
    }
    ...
    public function load_filecache(){
        mpf_require_class($this->filecache_class);
        $filecache=new $this->filecache_class();
        $cache_dir=@MPF::get_instance()->get_config("cache_dir","cache");
        if($cache_dir){
    ...
    private $memcache_class="MPF_Cache_Memcache";

    private $filecache_class="MPF_Cache_Filecache";

    private $redis_class="MPF_Cache_Redis";
