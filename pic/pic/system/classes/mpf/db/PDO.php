...
class MPF_DB_PDO extends PDO {
    ...
    public function exec($statement) {
        ...
        if (MPF::get_instance()->is_debug_enabled()) {
            MPF::get_instance()->debug(__CLASS__ .'['. $this->name .']'. "->exec: $statement");
        }
        ...
        if ($stmt instanceof PDOStatement) {
            ...
            if (parent::errorCode() !== '00000') {
                throw new MPF_Exception_SqlException(parent::errorCode(),$this->get_name().' | '.$this->config['dsn'].' | '.$statement.' | '.join(' | ',$error_info));
            }
        }
    ...
    public function prepare($statement, $driver_options=array()) {
        ...
        MPF::get_instance()->pf_benchmark("sql",array($this->i=>$statement));
    ...
    public function query($statement, $pdo=NULL, $object=NULL) {
        if (MPF::get_instance()->is_debug_enabled()) {
            MPF::get_instance()->debug(__CLASS__ .'['. $this->config['dsn'] .'|'.$this->name.']'. "->query: $statement");
        }
        $logger = MPF::get_instance()->get_logger();
