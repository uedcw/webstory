...
class MPF_Util_StringUtils {
     ...
     * $invert 保留、还是删除
     * $special 是否处理不完整的标签
     * $inner 删除时，是否保留inner文本
     */
     static public function strip_tags_content($text, $tags = '', $special = FALSE, $invert = FALSE, $inner = TRUE) {
         if(!$text)return $text;    
        $inner=($inner===FALSE)?'':"$2";
        ...
        if(is_array($tags) AND count($tags) > 0) {
            if($invert == FALSE) {
                $text = preg_replace('@<(?!(?:'. implode('|', $tags) .')\b)(\w+)\b.*?>(?<inner>.*?)</\1>@si', $inner, $text);
                if($special){
                    $text = preg_replace('@</?(?!(?:'. implode('|', $tags) .')\b)(\w+)\b.*?>@si', $inner, $text);
                }
                return $text;
            }
            else {
                $text = preg_replace('@<('. implode('|', $tags) .')\b.*?>(?<inner>.*?)</\1>@si', $inner, $text);
                if($special){
                    $text = preg_replace('@</?('. implode('|', $tags) .')\b.*?>', $inner, $text);
                }
                return $text;
            }
        }
        elseif($invert == FALSE) {
            return preg_replace('@<(\w+)\b.*?>(?<inner>.*?)</\1>@si', '$2', $text);
        }
        return $text;
    }
    
    static public function an_print($txt){
        if(is_string($txt))
    ...
    /*
     * gbk环境urlencode过来的参数，在utf-8环境使用时，必须转换编码
     */
    static public function iconv_urldecode($txt){
        if (!mb_check_encoding($txt, 'utf-8')) {
            $txt = @iconv('gbk', 'utf-8//IGNORE', $txt);
        }
        return $txt;
    }
    
    static public function str_word_count($txt){
        return preg_match_all('~[\p{L}\'\-\xC2\xAD]+~u', $txt);
    }
}
