...
class APF_Resource_ResourcesController extends APF_Controller{
    ...
    */
    public static function build_uri($resource){
        ...
        $type = @$apf->get_config(self::CONFIG_N_RESOURCE_TYPE_SINGLE,
            self::CONFIG_F_RESOURCE);
        if(!isset($type)){
            trigger_error("Unable to get config \""
    ...
    public function handle_request(){
        ...
        if('h'==$type){
            if($this->is_use_redis()){
                ...
                $allpatterns=$apf->get_config('mappings','route');
                $patterns=$allpatterns['APF_Resource_CompressedResources'];
    ...
    */
    ...
    static public function is_use_redis() {
        if(class_exists("redis") 
            && APF::get_instance()->get_config("use_hash", "resource") 
            && APF::get_instance()->get_config("redis", "cache")) {
            return true;
        } else {
            return false;
        }
