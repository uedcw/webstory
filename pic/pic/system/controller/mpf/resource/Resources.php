<?php
mpf_require_class("MPF_Controller");

class MPF_Resource_ResourcesController extends MPF_Controller{
    ...
    */
    public static function build_uri($resource){
        ...
        $version = @$mpf->get_config(self::CONFIG_N_VERSION,
            self::CONFIG_F_RESOURCE);
        $prefix = @$mpf->get_config(self::CONFIG_N_PREFIX_URI,
            self::CONFIG_F_RESOURCE);
        if(!isset($prefix)){
        ...
        $type = @$mpf->get_config(self::CONFIG_N_RESOURCE_TYPE_SINGLE,
            self::CONFIG_F_RESOURCE);
        if(!isset($type)){
            trigger_error("Unable to get config \""
        ...
        if(self::is_use_redis()){
            $ext=substr($resource,strpos($resource,'.')+1);
            ...
            if(false===$strContentHash){
                if(self::URL_FORCE_HASH){ //force hash
                    $o=new MPF_Resource_ResourcesController();
                    $strContentHash=$o->url2hash($type,$resource,'',$uri,true);
                    if(''==$strContentHash){
                        return $uri;
                    }
                    $type=$mpf->get_config('resource_type_hash',self::CONFIG_F_RESOURCE);
    ...
    */
    public static function build_boundable_uri($resource,$ext){
        ...
        $type = $mpf->get_config(self::CONFIG_N_RESOURCE_TYPE_BOUNDABLE,
            self::CONFIG_F_RESOURCE);
        if(!isset($type)){
            trigger_error("Unable to get config \""
        ...
        if(self::is_use_redis()){
            ...
            if(false===$strContentHash){
                if(self::URL_FORCE_HASH){ //force hash
                    ...
                    $type=$mpf->get_config('resource_type_hash',self::CONFIG_F_RESOURCE);
                    return $prefix.'/'.$type.'/'.$strContentHash.'.'.$ext.'#'.$resource.(isset($version)?'('.$version.')':'');
                }
                return $uri;
            }else{
                $type=$mpf->get_config('resource_type_hash',self::CONFIG_F_RESOURCE);
     ...
     * @see APF_Controller::handle_request()
     */
    public function handle_request(){
        ...
        if('h'==$type){
            if($this->is_use_redis()){
                mpf_require_class('MPF_Cache_Factory');
                $objRedis=MPF_Cache_Factory::get_instance()->get_redis('redis');
                $strURL=$objRedis->get('hash_'.$file);
                $allpatterns=$mpf->get_config('mappings','route');
                $patterns=$allpatterns['MPF_Resource_CompressedResources'];
                foreach($patterns as $pattern){
                    if(@ereg($pattern,$strURL,$matches)){
                        break;
                    }
                }
                $type=$matches[$idx_type];
    ...
    */
    ...
    static public function is_use_redis() {
        if(class_exists("redis") 
            && MPF::get_instance()->get_config("use_hash", "resource") 
            && MPF::get_instance()->get_config("redis", "cache")) {
            return true;
        } else {
            return false;
        }
