...
class MogileFSc
{
  ...
  public function __construct($domain, $class, $trackers, $memcacheds)
  {
    ...
    foreach ($memcacheds as $i) {
        $this->mc->addServer($i['server'], $i['port']);;
    }
  }
  ...
  public function getConnection()
  {
    ...
    foreach($this->trackers as $host)
    {
      ...
      if(!isset($parts['port']))
        $parts['port'] = MogileFSc::DEFAULT_PORT;
  ...
  {
    $res = $this->doRequest(MogileFSc::GET_DOMAINS);
  ...
  public function exists($key)
  {
    ...
    try {
      $this->doRequest(MogileFSc::GET_PATHS, Array('key' => $key));
      return true;
    } catch(Exception $e)
    {
      return false;
  ...
  public function getPaths($key)
  {
    ...
    $mckey = $this->domain . $this->class . $key;;
    $result = $this->mc->get($mckey);
    if ($result === FALSE) {
        $result = $this->doRequest(MogileFSc::GET_PATHS, Array('key' => $key));
        unset($result['paths']);

        $this->mc->set($mckey, $result, 0, 3600);
    }
    return $result;
  }
  ...
  public function delete($key)
  {
    ...
    $this->doRequest(MogileFSc::DELETE, Array('key' => $key));
    return true;
  }
  ...
  public function rename($from, $to)
  {
    ...
    $this->doRequest(MogileFSc::RENAME, Array('from_key' => $from, 'to_key' => $to));
    return true;
  }
  ...
  {
    try {
      return $this->doRequest(MogileFSc::LIST_KEYS, Array('prefix' => $prefix, 'after' => $lastKey, 'limit' => $limit));
    } catch(Exception $e)
    {
  ...
  public function setResource($key, $fh, $length)
  {
    ...
    $location = $this->doRequest(MogileFSc::CREATE_OPEN, Array('key' => $key));
    $uri = $location['path'];
    $parts = parse_url($uri);
    $host = $parts['host'];
    $port = $parts['port'];
    ...
    curl_close($ch);
    $this->doRequest(MogileFSc::CREATE_CLOSE, Array('key'   => $key,
  ...
  public function set($key, $value)
  {
    ...
    if(strlen(fread($fh,8192))==0){
        syslog(LOG_ERR,"MogilefsDebug:$key ".date("Y-m-d H:i:s"));
        throw new Exception(get_class($this) . "::set failed to write memory file");
    }
    rewind($fh);
